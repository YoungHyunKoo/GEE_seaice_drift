...
def retrieve_hourly_ERA5_bbox(year, months, days, bbox):
    c = cdsapi.Client(quiet=True, wait_until_complete=False, delete=True, progress=False, warning_callback = a, sleep_max=10)
    # dataset to read
    dataset = 'reanalysis-era5-single-levels'
    # flag to download data
...
def retrieve_monthly_ERA5(year, month, region = "SH"):
    c = cdsapi.Client(quiet=True, wait_until_complete=False, delete=True)
    # dataset to read
...
def rotate_vector_inv(u, v, lon, ref_lon = 0, hemi = "NH"):
    # Polar grid to Latitude & longitude grid
    if hemi == "NH":
        angle = (lon-ref_lon)*np.pi/180 # rotation angle (radian)
    else:
        angle = -(lon-ref_lon)*np.pi/180
...
def get_ERA5(ds, idx_era, xx, yy, region = "NH", ref_lon = 0):
    ...
    return grid_u10, grid_v10 #, grid_t2m, grid_sic, grid_i10
...
def derive_drift(a1, a2, t2, t1, pixel_size, directory = "D:\\Landfast\\ice_vel", velocity = True, pm_step = 40):
    ...
    srs = NSR(3857) #dstCrs = {'init': 'EPSG:32721'}
